                ...
                synchronized (this.lockTable) 
                {
                    ...
                    if (!bConflict) 
                    {
                        ...
                        if (bConvert.get(0) == true) 
                        {
                        	...
                        	{
                        		TrxnObj trxnReadObject = new TrxnObj(xid, strData, LockManager.READ);
                        		DataObj dataReadObject = new DataObj(xid, strData, LockManager.READ);
                        		
                        		trxnReadObject = (TrxnObj) lockTable.get(trxnReadObject);
                        		trxnReadObject.setLockType(LockManager.WRITE);
                        		dataReadObject = (DataObj) lockTable.get(dataReadObject);
                        		dataReadObject.setLockType(LockManager.WRITE);
 
                        		//trxObj.setLockType(LockManager.WRITE);
                        	}
                            // lock conversion 
